...
Нужен только Docker: ни Node, ни браузеров на машине ставить не надо. Внутри образа Node.js
...
Контекст сборки берётся из закоммиченного дерева, а не из рабочей копии. Так проверяется ровно то,
что получит любой клонировавший репозиторий: локальный `.env` не может ни попасть в образ, ни
замаскировать собственное отсутствие в чужом клоне.
...
после удаления контейнера, поэтому открываются командами раздела «Отчёты и трейсы» — Docker для
этого уже не нужен.

Allure целиком в контейнере: сервис собирает отчёт и раздаёт его на `http://localhost:8080`.

```bash
docker compose up allure
```

Именно `up`, а не `run`: при `docker compose run` порты сервиса не публикуются, пока не добавлен
флаг `-P`.

Настройки `ipc: host` и `init: true` заданы в описании сервиса, а не флагами командной строки — у
`docker compose run` таких флагов нет. Первая даёт Chromium разделяемую память сверх скромного
умолчания контейнера, вторая включает нормальную обработку сигналов; обе рекомендованы Playwright.

Прогон измерялся на Docker Desktop для macOS, где владелец записанных на хост файлов подставляется
сам. На хосте с Docker Engine под Linux пользователь образа (uid 1001) может не совпасть с
владельцем смонтированных каталогов; по документации Docker средство от этого — запуск с
`--user "$(id -u):$(id -g)"`.
