...
компилятор TypeScript в строгом режиме. Один пункт задания запускается по тегу
(`npx playwright test --grep @tc-1`), один слой — по проекту (`npx playwright test --project=api`).
...
нулём повторов: иначе он показывал бы flaky-тесты и опровергал сам себя. Единственное исключение —
сценарий регистрации: его повтор задан в самой спеке и, как измерено, перебивает и конфиг, и флаг
`--retries 0` в командной строке. То есть для этого теста «прогон без повторов» недостижим флагом, и
одиночное падение по вине стенда попадёт в сводку как `flaky`, а не как красный.
