...
- `npx playwright show-trace` открывает трейс локально. Тот же файл принимает
  [trace.playwright.dev](https://trace.playwright.dev) — по документации Playwright просмотрщик
  работает в браузере; сами мы этот путь не проверяли и на нём ничего не строим.
...
| Allure рядом со встроенным отчётом | только встроенный отчёт | Allure требуется заданием; взята третья версия — по документации она не требует Java, что важно для образа |
