...
7. Тело ответа разбирается схемой двумя способами. Через `expectValid`, когда предмет проверки и
   ...
   Потому тело с ошибками валидации идёт через `expectValid` в API-тестах, где проверяется сам
   ответ, и разбором в браузерной спеке, где проверяется то, что показала форма.
...
| спроектировано | `no-restricted-imports` против обхода слоёв                              | правило слоёв держится на ревью: при шести спеках нарушение видно глазами, а линтер пришлось бы настраивать под каждый каталог                                          |
