...
## Чего в наборе нет и куда он растёт дальше

Ниже — чего в наборе нет сегодня. Каждый пункт выглядит логичным следующим шагом: на объёме
тестового задания он не окупается, а с ростом набора становится нужен.

| Чего нет                                                            | Направление улучшения                                                                                                                                                                                                                                               |
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| структурированный логгер вместо четырёх `console.log`               | печать в stdout не проходит через маскирование — оно чистит архивы трейсов и вложения, — поэтому продуктовый логгер обязан звать `maskValues` сам; заодно вывод станет машиночитаемым                                                                               |
| мягкие проверки `expect.soft`                                       | дадут шагу сообщить все свои несоответствия сразу, а не только первое. Это мешает уже сейчас: в шаге проверки отказа падение ассерта о коде ответа прячет проверку состава полей, а в шаге о форме — проверку адреса страницы, и причина выясняется лишним прогоном |
| фикстура уникальных данных вместо ручного `uniqueId(parallelIndex)` | снимает целый класс ошибок на росте набора: сейчас слот передаётся руками, и `uniqueId(0)` в спеке скомпилируется молча. Фикстура знает свой `parallelIndex` сама                                                                                                   |
| фикстура-фабрика статей                                             | создавала бы статью и регистрировала её в уборке одним вызовом. Сейчас тест обязан не забыть `createdArticles.push(slug)` и сделать это до сравнений — в API-спеке об этом стоит отдельный комментарий                                                              |
| необязательное имя вложения у `expectValid`                         | имя выводится из схемы, а третий параметр остаётся только там, где нужно назвать природу данных, — как `article.broken.json` в ТК4                                                                                                                                  |
