...
4. Когда дефект починят, первый же прогон краснеет с текстом `Expected to fail, but passed.`:
   метку пора снимать. Это строгий режим, как `xfail(strict=True)` в pytest (по его документации).
...
прогон, и перечисление тестов, называя переменную. Шаблон пустых значений — `.env.example`.
...
6. Все строки, попадающие в отчёт, — на английском: заголовки, шаги, сообщения проверок, имена
   вложений.
7. Тело ответа разбирается схемой двумя способами. Через `expectValid`, когда предмет проверки и
   есть форма продуктового объекта: он приложит замаскированный снимок и назовёт последствие. Голым
   `Schema.parse`, когда схема лишь даёт типизированный доступ к полю, а проверяет следующий ассерт.
   Потому тело с ошибками валидации идёт через `expectValid` в API-тестах, где проверяется его
   форма целиком, и разбором в браузерной спеке, где из него берётся только список полей.
...
Вывод настоящего прогона на четырёх воркерах; имена пользователей убраны.
...
Если тот же набор понадобится в GitLab CI, шаги переносятся один в один — по документации GitLab, в
самом GitLab это не прогонялось: `npm ci` — в `default:before_script`, на верхнем уровне ключ
помечен устаревшим; `npm run lint`, `npm run typecheck` и `npm run test:unit` — в задачу `static`
...
`when: always`; отмена предыдущего прогона ветки — `interruptible: true` у задачи вместе с
`on_new_commit: interruptible` в `workflow:auto_cancel`. Готового `.gitlab-ci.yml` в репозитории нет
намеренно: непроверенный конфиг — это обещание, за которое никто не отвечал.
...
## Чего во фреймворке намеренно нет

Границы у набора выбраны, а не забыты. Первая группа спроектирована в `ARCHITECTURE.md` и
сознательно не построена здесь, вторая нужна продукту, но не этому заданию.

| Группа         | Чего нет                                                                 | Почему нет                                                                                                                                                              |
| -------------- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| спроектировано | контрактный слой page objects с опцией `appVersion`                      | приложение одно и одной версии: интерфейс с единственной реализацией — абстракция с единственным потребителем                                                           |
| спроектировано | матрица сред, версий и браузеров                                         | один стенд и один браузер; задание просит стабильность и скорость, а не покрытие матрицей                                                                               |
| спроектировано | API тестовых данных с массовой уборкой по идентификатору прогона         | публичный стенд такого API не даёт: убирать приходится постатейно в teardown                                                                                            |
| спроектировано | обязательные секреты в схеме и их требование проверяющим проектом        | сама схема и проверяющий проект здесь есть (`src/config/env.ts`, `tests/setup/env.setup.ts`); нет секретов — публичному стенду не нужен ни один                         |
| спроектировано | реестр test-id и правило `playwright/no-raw-locators` (по README пакета) | в приложении нет ни одного test-id, а его фронтенд нам не принадлежит: правилу было бы нечего охранять, кроме запрета на всё                                            |
| спроектировано | `no-restricted-imports` против обхода слоёв                              | правило слоёв держится на ревью: при сегодняшнем размере набора нарушение видно глазами, а линтер пришлось бы настраивать под каждый каталог                            |
| нужно продукту | структурированный логгер вместо четырёх `console.log`                    | важнее самого логгера то, что маскирование секретов сегодня покрывает архивы трейсов и вложения, но не stdout: продуктовый логгер обязан идти через тот же `maskValues` |
| нужно продукту | шардирование и слияние отчётов                                           | весь набор идёт около тридцати секунд; делить нечего, а машинерия слияния добавила бы то, что придётся чинить                                                           |
| нужно продукту | история прогонов и накопление нестабильности                             | необязательная часть, и она не обещана: для истории нужен хранимый между прогонами каталог, то есть инфраструктура, а не код набора                                     |
| нужно продукту | ролевая модель с разными `storageState`                                  | в Conduit одна роль пользователя; вторая появилась бы без предметной надобности                                                                                         |
| нужно продукту | мягкие проверки `expect.soft`                                            | там, где форма отвечает несколькими ошибками, ассерт и так сравнивает список целиком, поэтому первое падение ничего не скрывает                                         |
| нужно продукту | проверки доступности и вида                                              | чужое приложение, чужая вёрстка: эталон был бы снят с того, что мы не контролируем, и краснел бы на чужих правках                                                       |
| нужно продукту | фикстура уникальных данных вместо ручного `uniqueId(parallelIndex)`      | правка на вырост: трогает все спеки разом, а на четырёх воркерах столкновений слотов не наблюдалось                                                                     |
| нужно продукту | фикстура-фабрика статей                                                  | та же цена и та же выгода: при сегодняшнем числе мест создание статьи через слой API читается не хуже                                                                   |
| нужно продукту | необязательное имя вложения у `expectValid`                              | обязательный параметр — лишний литерал, а не дефект; правка тронула бы все вызовы ради минус одного слова                                                               |
