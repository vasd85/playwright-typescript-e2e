...
вместе с трейсом упавшего теста на `http://localhost:9323`; обе проверены запросом с хоста. Флаг
`-p` обязателен и здесь: порт, объявленный сервисом, при `docker compose run` сам не публикуется —
проверено, без флага с хоста не отвечает ничто.
