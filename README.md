...
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| структурированный логгер вместо четырёх `console.log`               | закрывает дыру в маскировании: секреты вычищаются из архивов трейсов и вложений, но не из stdout, а логгер провёл бы вывод через тот же `maskValues` — и заодно сделал бы его машиночитаемым                              |
| мягкие проверки `expect.soft`                                       | покажут все несоответствия шага за один прогон вместо первого. Понадобятся, когда в шаге проверяются независимые друг от друга вещи: сегодня список полей формы сравнивается целиком одним ассертом, и прятать ему нечего |
...
| фикстура-фабрика статей                                             | собирает создание статьи и её уборку в один объект, чтобы тест не помнил про teardown; окупается, когда таких мест станет заметно больше сегодняшних                                                                      |
